...
import { GET_AUTHORS, GET_POSTS, REMOVE_AUTHOR } from "./query";
...
export const ListAuthors = (props: any) => {
  ...
  const [removeAuthor] = useMutation(REMOVE_AUTHOR, {
    refetchQueries: [{ query: GET_AUTHORS }, "getAuthors"],
  });
            ...
            (author: Author) =>
              !!author && (
                <React.Fragment key={author._id}>
                  ...
                  <button onClick={(event) => handleRemove(event, author._id)}>
                    Remove
                  </button>
                  <button
                    onClick={(event) => handleUpdate(event, author)}
